      ...
      >
        ...
        >
          <Text style={{ fontSize: 18 }}>
            Total cases: {props?.item?.cases?.total}
          </Text>
          <View style={{ flexDirection: "row" }}>
            <Text style={{ fontSize: 18 }}>
              {props?.item?.cases?.new ?? "N/A"}
            </Text>
            {props.item?.cases?.new && (
              <Icon
                style={{ marginLeft: 4 }}
                color={"red"}
                size={18}
        ...
        >
          <Text style={{ fontSize: 14 }}>
            Total deaths: {props?.item?.deaths?.total}
          </Text>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <Text style={{ fontSize: 14 }}>
              {props?.item?.deaths?.new ?? "N/A"}
            </Text>
      ...
      <SafeAreaView>
        ...
        <ScrollView>
          {currentCountryObject && (
            <TouchableOpacity
              onPress={() => onListItemPress(currentCountryObject)}
            >
              <ListItemCard item={currentCountryObject}></ListItemCard>
            </TouchableOpacity>
          )}
          {(search ? searchResult : list)?.sort(sortList).map((item, index) => (
            <TouchableOpacity key={index} onPress={() => onListItemPress(item)}>
              <ListItemCard key={index} item={item}></ListItemCard>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </SafeAreaView>
